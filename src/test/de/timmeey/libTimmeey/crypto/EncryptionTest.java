...
public class EncryptionTest {
	...
	@Test
	public void testEncryption() throws InvalidKeyException,
			NoSuchAlgorithmException, NoSuchPaddingException,
			IllegalBlockSizeException, BadPaddingException {
		String testString = UUID.randomUUID().toString();
		byte[] encryptedString = Encryption.encrypt(testString.getBytes(),
				publicKey);
		byte[] decryptedString = Encryption.decrypt(encryptedString, privatKey);
		assertEquals("Encrypted and decrypted string should be the same",
				testString, new String(decryptedString));

	}

	@Test
	public void testSigning() throws InvalidKeyException,
			NoSuchAlgorithmException, NoSuchPaddingException,
		...
		assertEquals("Should have failed due to a wrong signature", false,
				Encryption.checkSignature(wrongTestString.getBytes("UTF-8"),
						signedByte1, publicKey));
