...
public class SimpleObjectPool<K, V> extends TimerTask implements
	 ...
	 */
	public SimpleObjectPool(@NonNull Long idleTimeout, @NonNull Long
		cleanUpTime,
			Verifier<V> verifier) {
		logger.debug(
	...
	private ObjectPool<K, V> internalRemove(K key) {
		...
		internalRemove(new ArrayList<>(Arrays.asList(key)));
	...
	private void putBack(V object, boolean resetTime) {
		...
		K key = hashToKey.get(object.hashCode());
		logger.trace(
			"Object is returned back.The old key is: {}. idleTime Should be " +
				"resettet: {}",
			key, resetTime);
		if (key == null) {
			logger.debug("Could not find any old key for the returned object, " +
				"discarding it");
			return;
		}
